...
The first thing I have thought about was to generated from a specific distribution family. For example, spatial auto-regression, Matern, et cetera. In such cases, the covariance/correlation between two pairs of spatial points would be a function of the "distance" between the two points. 

On this point, I am struggling with the dimensionality problem of spatial index. Let's say we are working on a 2D space $(s_1, s_2)$: 
...
Even though we have only `r length(s1)` points along each axis, there would be `r nS` points, thus `r nS^2` distance measures. 


```{r}
# calculate distance
coords$pid <- 1:nS
df_dist <- expand.grid(p1 = 1:nS, p2 = 1:nS) %>% 
  mutate(dist=NA)

for(i in 1:nrow(df_dist)){
  coord_p1 <- coords[coords$pid==df_dist$p1[i], c("Var1", "Var2")]
  coord_p2 <- coords[coords$pid==df_dist$p2[i], c("Var1", "Var2")]
  df_dist$dist[i] <- sqrt(sum((coord_p1-coord_p2)^2))
}
```

```{r}
# Matern correlation matrix
sp_cormat <- matern(df_dist$dist, phi = 0.25, kappa = 0.25)
plot(df_dist$dist, sp_cormat, type = "p", main = "Matern correlation",
     xlab = "Distance", ylab = "", pch=20, cex = 0.5)
df_dist$cor <- sp_cormat
```

```{r}
sp_cormat <- matrix(sp_cormat, nrow = nS, ncol = nS) # row-p1, col=p2
heatmap(sp_cormat, Rowv=NA, Colv = NA)
```
...
2. Spatial effect follows a Matern distirbution

https://en.wikipedia.org/wiki/Mat%C3%A9rn_covariance_function
3. Temporal effect is a slope
...
But how can I calculate the correlation averaging over space given time and subject? That is, the correlation betwee $Y_i(.,t)$ and $Z_i(., t)$? 

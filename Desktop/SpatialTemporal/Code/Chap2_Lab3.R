...
## ------------------------------------------------------------------------
head(E$u[, 1:6]) # principal component time series, data projected on to EOR space
...
## ------------------------------------------------------------------------
TS$nPC <- TS$PC * sqrt(nT-1) # normalize
...
ggplot(EOFs) + geom_tile(aes(x = lon, y = lat, fill = EOF3)) +
  fill_scale(name = "degC") + theme_bw() +
  xlab("Longitude (deg)") + ylab("Latitude (deg)")
...
cor(CCA_df$CCAvar1, CCA_df$CCAvar2)
...
dim(EOF)
df_wt <- cbind(EOFs[, 1:2], EOFset1[, 1:10] %*% cc$xcoef)
colnames(df_wt) <- c("lon", "lat", paste0("wt", 1:10))
ggplot(df_wt)+
  geom_tile(aes(x = lon, y = lat, fill = wt1)) +
